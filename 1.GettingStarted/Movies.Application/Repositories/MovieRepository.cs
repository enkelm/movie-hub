...
public class MovieRepository : IMovieRepository
{
    ...
    public async Task<IEnumerable<Movie>> GetAllAsync(GetAllMoviesOptions options, CancellationToken token = default)
    {
        ...
        if (options.SortField is not null)
        {
            orderClause = $"""
            , m. {options.SortField}
            oreder by m.{options.SortField} {(options.SortOrder == SortOrder.Ascending ? "asc" : "desc")}
            """;
        }
            ...
            """, new
        {
            ...
            title = options.Title,
            yearorrelease = options.YearOfRelease,
    ...
    public async Task<bool> UpdateAsync(Movie movie, CancellationToken token = default)
    {
        ...
        var result = await connection.ExecuteAsync(new CommandDefinition("""
            update movies set slug = @Slug, tile = @Title, yearofrelease = @YearOfRelease
            where id = @Id
            """, movie, cancellationToken: token));
    ...
    public async Task<int> GetCountAsync(string? title, int? yearOfRelease, CancellationToken token = default)
    {
        ...
        return await connection.QuerySingleOrDefaultAsync<int>(new CommandDefinition("""
            ...
            where (@title is null or tiltle like('%' || @title || '%'))
